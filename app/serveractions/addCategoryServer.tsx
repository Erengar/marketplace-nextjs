...
const category = z.object({
    ...
    description: z.string({invalid_type_error: "Invalid description"}).max(1000, "Description must contain at most 1000 character(s)."),
})
...
export async function addCategoryServer(prevState: any, formData: FormData) {
    ...
    if (!data.success) {
        ...
        try{
            ...
            await db.insert(categories).values({name: name, description: description, slug: slug})
        } catch (e: any) {
            if (e.code === '23505') {
                return {
