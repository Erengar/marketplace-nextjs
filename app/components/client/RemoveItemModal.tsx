...
export default function RemoveItemModal({ removingItem, setRemovingitem }: RemoveItemModalProps) {

    // This function removes the item from the shopping cart
    function removeItem(){
        const shoppingCart = localStorage.getItem('shoppingCart')
        const cartItems = shoppingCart ? JSON.parse(shoppingCart) : []
        const item = cartItems.find((item: CartItemType) => item.product.id === removingItem.product.id)
        if (item) {
